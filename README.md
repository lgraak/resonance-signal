...
The foundation, audio-data-contract, basic signal-processing, bounded-window-scheduling, stereo-first capture-requirements, capture-backend-selection, Windows playback-loopback prototype, real-device validation, Windows capture-boundary productionization, capture-owner lifecycle, capture-supervisor-boundary design, recovery-disabled capture-supervisor state, recovery-decision-policy design, recovery-policy-representation, retry-state-policy-design, and retry-state-transition milestones are complete. The workspace defines provider-independent waveform and derived signal frames, a transport-neutral multi-source contract, bounded analysis cadence with explicit discontinuity handling, zero-copy waveform subwindows, RMS and peak levels, explicit peak normalization, and a lifecycle-managed Windows playback-capture component in `resonance-agent`. The Windows adapter uses `wasapi` 0.24.0 to turn the default playback endpoint into bounded, validated `AudioFrame` and `StreamEvent` output with explicit owner and supervisor state, startup, bounded shutdown/completion waiting, completion reporting, and overload behavior. `CaptureSupervisor` owns capture intent and at most one single-use `CaptureOwner`, observes terminal lifecycle and joined completion, and records whether the resource-release boundary required by future recovery has been reached. Agent-internal modules implement ADR 0008's pure recovery decisions and ADR 0009's deterministic retry state: typed intent generations, once-per-owner-creation attempt accounting, bounded failure history, recovery episodes, evidence-only cooldown, sticky exhaustion, immutable snapshots, and stale-snapshot rejection. The retry state and policy remain disconnected from runtime supervisor execution and never create a replacement owner. Automatic reconnect, retry scheduling, backoff execution, endpoint watching, and replacement capture remain unimplemented. Microphone and Linux capture, FFT processing, device discovery, serialization, service installation, and service transports are also not implemented.
...
- `resonance-agent`: platform capture ownership, the private recovery policy and retry-state model, the production Windows playback-loopback adapter, provider-event orchestration, and the diagnostic executable.
...
The diagnostic creates a `CaptureSupervisor`, which creates and starts one `CaptureOwner`, waits for the requested duration, then disables running intent, requests owner stop, and waits up to two seconds for joined completion. A supervisor stop before `start` creates no owner, repeated stop requests are harmless, and a completion timeout retains ownership so the caller can wait again. `CaptureOwner::wait_for_completion` also permits natural completion to be observed without implicitly requesting stop. Buffer-pool depth, maximum packet allocation, and WASAPI event-wait cadence remain internal implementation details. The supervisor is intentionally recovery-disabled: the retry transition model and pure recovery policy are tested state components, not wired execution. No transition sleeps, observes a device, invokes a factory, or creates an owner. A future replacement would require separately approved supervisor integration and execution, and would produce a new stream identity and timeline.
