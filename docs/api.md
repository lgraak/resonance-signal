...
Frames are bounded, independently owned batches. The local v1 transport schedules them into complete windows and uses a bounded per-client queue; overflow terminates the affected consumer rather than silently accumulating data.
...
Selected playback devices, microphones, and virtual devices are all addressed by opaque ID. `SourceKind` describes the resolved source as playback, microphone, virtual, or other. The accepted identity domain is one provider installation on one host with one retained mapping namespace. Within that domain, a provider-managed `SourceId` remains stable across discovery refreshes, process restarts, metadata and default-role changes, and temporary absence when native continuity is proven. It is not portable across hosts, installations, mapping resets, operating-system reinstalls, or unproven backend re-enumeration. Retired IDs are never reassigned, and ambiguous return requires rediscovery rather than guessed continuity. Source selection is recorded in [ADR 0013](decisions/0013-source-selection-model.md); discovery and scoped identity are recorded in [ADR 0014](decisions/0014-source-discovery-and-identity-model.md); the consumer descriptor and private registry contract are recorded in [ADR 0015](decisions/0015-consumer-discovery-and-identity-registry.md).

`DiscoverySnapshot` is an owned, replaceable point-in-time result containing an opaque equality-only `DiscoveryRevision` and a complete descriptor set. Each `SourceDescriptor` reports an opaque `SourceId`, `SourceKind`, optional non-empty display name, three-state `SourceAvailability` (`Available`, `Unavailable`, or `Unknown`), a duplicate-free `SignalProduct` set, and duplicate-free current `DefaultSource` roles. The Windows mapping advertises only `Playback`, `Waveform`, and current Default Playback membership. Display names, availability, default roles, formats, transport classes, and capabilities are mutable metadata, not identity. Source ordering has no identity meaning. Absence from a snapshot does not by itself invalidate an ID, and a default-role annotation neither reserves the endpoint nor converts explicit selection into Default Playback intent.

A durable Explicit Source intent may retain only its `SourceId`, but current registry identity and availability are always revalidated and no other source may be substituted. `resonance-agent` implements the private registry storage contract and Windows playback discovery snapshot: explicit refresh enumerates active render endpoints, maps persistent `IMMDevice` endpoint evidence to opaque IDs, records the console default-role owner, retains known absent sources as unavailable, reports only proven waveform support, rejects stale private snapshots, and resolves an unavailable explicit ID without substitution. Its portable conversion owns every returned value and strips native endpoint IDs, continuity evidence, registry namespaces, tombstones, schemas, and storage details. Default Playback and Explicit Source capture consume separate intent paths over the same private mapping at attempt start. The v1 loopback transport publishes complete snapshots and accepts either current Default Playback intent or a durable exact `SourceId`; paging, change delivery, remote authorization, and snapshot-precondition selection remain deferred.
...
- discovery enumeration, snapshot representation, and capability negotiation;
- v1 serialization field names, framing, and compatibility before wider client adoption;
- remote transport, authentication, authorization, and TLS;
- wall-clock correlation between streams or hosts.

Backend selection is recorded in [ADR 0004](decisions/0004-capture-backend-selection.md), the production Windows lifecycle/buffering policy is recorded in [ADR 0005](decisions/0005-windows-capture-lifecycle-and-buffering.md), the explicit capture-owner state machine is recorded in [ADR 0006](decisions/0006-capture-owner-lifecycle.md), the supervisor boundary is recorded in [ADR 0007](decisions/0007-capture-supervisor-boundary.md), the recovery decision model is recorded in [ADR 0008](decisions/0008-recovery-policy-boundary.md), retry-state ownership and accounting are recorded in [ADR 0009](decisions/0009-retry-state-and-policy-boundary.md), recovery-configuration validation and identity are recorded in [ADR 0010](decisions/0010-recovery-configuration-model.md), operational recovery parameter policy is recorded in [ADR 0011](decisions/0011-operational-recovery-parameters.md), source intent and selection semantics are recorded in [ADR 0013](decisions/0013-source-selection-model.md), discovery plus scoped source identity are recorded in [ADR 0014](decisions/0014-source-discovery-and-identity-model.md), and local transport is recorded in [ADR 0016](decisions/0016-local-consumer-transport.md). Platform types, recovery parameters, native source identifiers, and dependencies remain private to `resonance-agent`.
...
`run_playback_loopback` and its `CaptureStopToken` are the blocking adapter seam owned by `CaptureOwner`; `run_default_playback_loopback` remains the Default Playback convenience entry point. `run_default_playback_loopback_for` remains available as a bounded diagnostic helper, but the command-line executable validates the supervisor and owner lifecycle together: it starts an indefinite run, waits for `--duration-seconds`, requests supervisor stop, and allows two seconds for completion. The duration accepts 1 through 3600 seconds and defaults to 10; `--source-id <opaque-id>` selects Explicit Source instead of Default Playback. These are diagnostic options, not a stable transport or backend tuning surface. At each attempt start, Default Playback refreshes the console-role observation, while Explicit Source refreshes and resolves exactly its requested live registry identity. Both bind the opaque `SourceId` to one private endpoint ID, open that endpoint directly, and revalidate the complete binding after endpoint/session notification registration and before publishing `Started`. Explicit capture does not subscribe to default-role movement as a stream boundary. Missing registry durability, unknown/unavailable/retired identity, unavailable default role, stale binding, or opened-endpoint mismatch fails before a stream descriptor is published. The descriptor reports exactly the mapped opaque `SourceId`; neither logical intent nor native endpoint identity is emitted. The diagnostic uses private registry state under `%LOCALAPPDATA%\Resonance Signal\provider-state`. Automatic reattempts and in-place source migration remain absent.
...
The local transport is recorded in [ADR 0016](decisions/0016-local-consumer-transport.md) and specified byte-for-byte in [Consumer Protocol](consumer-protocol.md). Its versioned JSON and binary schemas preserve this semantic contract rather than exposing Rust or backend memory layout.
