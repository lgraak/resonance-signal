...
The capture owner does not reconnect automatically and cannot be restarted. Recovery policy must create a new owner explicitly after inspecting the machine-actionable error kind, retry hint, end reason, and completion state. Every later capture run creates a new `StreamId`; its first accepted frame starts at frame index zero and timestamp zero. Startup and runtime failures expose platform-neutral categories separately from non-stable human diagnostic text. Consumers must not parse console or `Display` output.
