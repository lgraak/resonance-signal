...
A future consumer-facing `DiscoverySnapshot` is a replaceable point-in-time result containing an opaque equality-only revision and a complete descriptor set. Each `SourceDescriptor` conceptually reports an opaque `SourceId`, `SourceKind`, optional non-empty display name, three-state availability (`Available`, `Unavailable`, or `Unknown`), the supported `SignalProduct` set, and current logical default-role membership. Display names, availability, default roles, formats, transport classes, and capabilities are mutable metadata, not identity. Absence from a snapshot does not by itself invalidate an ID, and a default-role annotation neither reserves the endpoint nor converts explicit selection into Default Playback intent.

A future operation made directly from a snapshot supplies the chosen ID and snapshot revision. Any observation or registry change makes the precondition stale and causes rejection before capture resolution. A durable Explicit Source intent may retain only its `SourceId`, but current registry identity and availability are always revalidated and no other source may be substituted. `resonance-agent` now implements the private registry storage contract and a private Windows playback discovery snapshot: explicit refresh enumerates active render endpoints, maps persistent `IMMDevice` endpoint evidence to opaque IDs, records the console default-role owner, reports only proven waveform support, rejects stale snapshots, and resolves an unavailable explicit ID without substitution. Public discovery request/result types, consumer transport, change delivery, authorization, capture-time registry integration, and explicit-source capture remain implementation work.
