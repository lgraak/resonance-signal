...
A future operation made directly from a snapshot supplies the chosen ID and snapshot revision. Any observation or registry change makes the precondition stale and causes rejection before capture resolution. A durable Explicit Source intent may retain only its `SourceId`, but current registry identity and availability are always revalidated and no other source may be substituted. `resonance-agent` implements the private registry storage contract and Windows playback discovery snapshot: explicit refresh enumerates active render endpoints, maps persistent `IMMDevice` endpoint evidence to opaque IDs, records the console default-role owner, retains known absent sources as unavailable, reports only proven waveform support, rejects stale private snapshots, and resolves an unavailable explicit ID without substitution. Its portable conversion owns every returned value and strips native endpoint IDs, continuity evidence, registry namespaces, tombstones, schemas, and storage details. Default Playback and Explicit Source capture now consume separate intent paths over the same private mapping at attempt start. Consumer transport, change delivery, and authorization remain implementation work.
