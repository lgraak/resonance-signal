...
Backend selection is recorded in [ADR 0004](decisions/0004-capture-backend-selection.md), and the production Windows lifecycle/buffering policy is recorded in [ADR 0005](decisions/0005-windows-capture-lifecycle-and-buffering.md). Neither decision changes the consumer contract: platform types and dependencies remain private to `resonance-agent`.
...
`run_default_playback_loopback` is the production lifecycle entry point. It runs until its cloneable `CaptureStopToken` is requested or the current uninterrupted stream reaches a boundary. `run_default_playback_loopback_for` is the bounded diagnostic helper used by the command-line executable. The `--duration-seconds` value accepts 1 through 3600 seconds and defaults to 10; it is testing configuration, not a backend tuning parameter. Source selection remains the default console playback endpoint until discovery and explicit selection are designed.
...
The capture boundary does not reconnect automatically. Every later invocation creates a new `StreamId`; its first accepted frame starts at frame index zero and timestamp zero. The owner decides whether and when to invoke capture again after inspecting the machine-actionable error kind, retry hint, and end reason. Startup and runtime failures expose platform-neutral categories separately from non-stable human diagnostic text. Consumers must not parse console or `Display` output.
