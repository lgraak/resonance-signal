...
The agent-internal recovery policy represents typed intent generations, stable recovery causes, lifecycle completion, source-selection and availability evidence, and bounded retry-state inputs. Its pure evaluator implements ADR 0008 precedence and returns only remain-stopped, wait, or permit-replacement with a stable reason. Explicit stop and stale intent win; incomplete cleanup blocks replacement; missing or inconsistent structured evidence fails closed. Device loss, reconfiguration, interruption, and resource exhaustion are conditional cases. Unsupported format under unchanged conditions, a coarsely classified startup failure, broad internal failure, and worker panic remain stopped. Retry hints constrain a decision but are never commands.

The retry transition model is also private to `resonance-agent`. It records one nonzero intent generation per explicit start and one monotonic attempt identity per committed owner-creation call. Owner creation, owner start, `Started`, terminal failure, terminal delivery, and joined cleanup are lifecycle facts on that same attempt; none increments it again. Total attempts and automatic recovery attempts remain separate, and a successful start does not reset the recovery episode or its budget accounting.

`RetryState` transitions through `Idle`, `Attempting`, `Running`, `Failed`, `Waiting`, `Exhausted`, and `Stopped`. `Failed` retains incomplete terminal or cleanup evidence; only complete evidence reaches `Waiting`. Its immutable owned snapshot binds intent generation, state revision, policy-configuration identity, episode, and current attempt, so a later mutation makes the snapshot stale. Recent typed failures have a structural capacity bound while aggregate counts remain monotonic. Exhaustion is sticky within an episode and only explicit validated reset evidence advances the episode.
...
These policy and retry types remain private to `resonance-agent` and do not change `resonance-api`. The current runtime `CaptureSupervisor` does not yet own or invoke these components, and neither component can create or stop owners, sleep, inspect hardware, watch endpoints, or emit consumer events. Automatic reconnect, retry/backoff execution, endpoint watching, default-device following, and replacement creation remain unimplemented.
