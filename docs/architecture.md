...
A future discovery snapshot conceptually exposes source ID, kind, optional display label, current availability when known, and point-in-time default-role annotations. It neither reserves a source nor guarantees that metadata remains current. Backend-native endpoint or node identifiers stay private, retired IDs are never recycled, and missing or ambiguous return evidence invalidates continuity and requires rediscovery.

Disappearance, replacement, return, reconfiguration, and format change always end the current stream. A later attempt starts a new stream at frame index and stream-relative time zero. Default Playback resolves the then-current role owner; Explicit Source accepts only the same proven identity. Enumeration, mapping persistence, endpoint watching, and recovery execution remain deferred.
...
- Source identity is scoped to one installation-on-one-host mapping domain, but mapping storage, migration, reset, and platform continuity validation are not implemented.
