...
- The Windows discovery adapter maps endpoint identity to the consumer-visible `SourceId`, but the capture adapter still emits its legacy `default-playback` placeholder and does not consume discovery revisions or explicit IDs.
