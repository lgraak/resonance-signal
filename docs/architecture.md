...
Owns the transport-independent consumer-facing semantic contract: portable discovery snapshots and descriptors, source selection, subscriptions, stream lifecycle, product payloads, contract versioning, and platform-neutral failures. It depends on and re-exports `resonance-core` signal types.
...
The consumer `DiscoverySnapshot` exposes an opaque equality-only revision plus owned descriptors containing source ID, kind, optional display name, three-state availability, supported signal products, and point-in-time default roles. It neither reserves a source nor guarantees that metadata remains current. A future direct selection from a snapshot carries the revision as a freshness precondition; revision mismatch is rejected before resolution. Backend-native endpoint or node identifiers stay private, retired IDs are never recycled, and missing or ambiguous return evidence invalidates continuity and requires rediscovery.

`resonance-agent` exclusively owns the durable identity registry. One logical writer atomically commits installation-and-host-bound namespace state, live and absent mappings, registry revision, private backend evidence, schema identity, and permanent retired-ID tombstones. Process restart and compatible provider upgrades retain identity; host change, namespace loss, corruption, incompatible schema, failed migration, or reset safely loses continuity rather than guessing. A new namespace must be durably committed before discovery can publish IDs, and failed persistence leaves discovery unavailable instead of emitting ephemeral identities. The agent converts one committed private snapshot to the portable API value, retaining known non-retired absent sources as `Unavailable` while excluding native keys, continuity tokens, registry namespaces, storage state, and tombstones.

Windows discovery calls `IMMDeviceEnumerator::EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE)` through `wasapi` 0.24.0 and defensively rechecks each returned endpoint state. Only `Active` render endpoints create or restore live mappings; disabled, not-present, unplugged, capture, and microphone observations do not. A previously known playback source absent from the active enumeration remains describable as `Unavailable` until it returns or is retired. The adapter resolves `GetDefaultAudioEndpoint(eRender, eConsole)` and annotates only the matching active descriptor. Default Playback remains role intent and resolves through that annotation to the endpoint's mapped `SourceId`; it never receives a synthetic identity.
...
Disappearance, replacement, return, reconfiguration, and format change always end the current stream. A later attempt starts a new stream at frame index and stream-relative time zero. Default Playback resolves the then-current role owner; Explicit Source accepts only the same proven identity. Refresh is explicit and bounded; continuous watching, microphone discovery, PipeWire, remote transport, and recovery execution remain deferred.

The Windows capture adapter retains an agent-level `DefaultPlayback | Explicit(SourceId)` intent for each attempt. Default Playback resolves the current console-role owner through private discovery and the identity registry; Explicit Source resolves only the requested current live registry identity. One private binding carries the mapped opaque `SourceId`, exact WASAPI endpoint ID, and discovery revision. Capture opens that endpoint by ID, then refreshes and compares the same intent-specific binding after endpoint/session notification registration. Default-role movement is relevant only to Default Playback; an explicit stream remains pinned to its endpoint until that endpoint itself ends, disappears, or reconfigures. An unavailable role, unknown/unavailable/retired ID, registry failure, stale revision, changed binding, or endpoint mismatch fails before `StreamEvent::Started`. The descriptor reports the mapped `SourceId`; logical intent and native endpoint ID remain private. The current diagnostic runtime keeps registry state under the current user's local application data at `Resonance Signal\provider-state`.
...
is considered active only when its strict quoted command targets the current
executable. Installer, updater, Windows Service, and admin-elevated lifecycle
ownership remain outside the architecture.
...
The scheduler retains less than one window between calls and bounds accepted work by a configured maximum number of windows per push. It emits immediately on completion, retains partial slow input without padding or timeout flushing, and rejects oversized calls without mutating state. The v1 local transport adds its own bounded queue, session limit, and slow-consumer termination policy without changing scheduler behavior.
...
- FFT, spectrum generation, frequency bands, smoothing, and overlapping hops are deferred.
- Consumer discovery has a complete loopback HTTP representation; paging, filters, change subscription, and remote authorization are absent.
- The Windows capture adapter supports revision-bound Default Playback and explicit-ID mappings and emits the selected registry-backed `SourceId`; it does not watch endpoints or migrate active streams.
- Source identity is scoped to one installation-on-one-host mapping domain. Registry storage and Windows continuity validation are implemented privately; public registry administration, export, and migration between hosts remain unsupported.
- No non-loopback service, remote authentication/TLS design, or service-manager installation is defined.
...
See [ADR 0001](decisions/0001-audio-data-contract.md) for the audio contract, [ADR 0002](decisions/0002-bounded-window-scheduling.md) for scheduling and buffering decisions, [ADR 0003](decisions/0003-stereo-first-capture-boundary.md) for capture scope and enforcement, [ADR 0004](decisions/0004-capture-backend-selection.md) for backend evidence and implementation direction, [ADR 0005](decisions/0005-windows-capture-lifecycle-and-buffering.md) for the production Windows lifecycle and bounded handoff, [ADR 0006](decisions/0006-capture-owner-lifecycle.md) for explicit owner/thread/shutdown responsibility, [ADR 0013](decisions/0013-source-selection-model.md) for source intent and selection semantics, [ADR 0014](decisions/0014-source-discovery-and-identity-model.md) for discovery and scoped source identity, [ADR 0015](decisions/0015-consumer-discovery-and-identity-registry.md) for the portable snapshot and private registry boundary, and [ADR 0016](decisions/0016-local-consumer-transport.md) for the loopback HTTP/WebSocket service.
