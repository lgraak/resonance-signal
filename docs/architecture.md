...
Disappearance, replacement, return, reconfiguration, and format change always end the current stream. A later attempt starts a new stream at frame index and stream-relative time zero. Default Playback resolves the then-current role owner; Explicit Source accepts only the same proven identity. Refresh is explicit and bounded; continuous watching, microphone discovery, PipeWire, consumer transport, explicit-source capture, and recovery execution remain deferred.

The Windows capture adapter resolves Default Playback through the private discovery and identity registry at each attempt start. One private binding carries the mapped opaque `SourceId`, exact WASAPI endpoint ID, and discovery revision. Capture opens that endpoint by ID rather than re-resolving the default independently, then refreshes and compares the binding after endpoint/session notification registration. An unavailable role, registry failure, stale revision, changed binding, or endpoint mismatch fails before `StreamEvent::Started`. The descriptor reports the mapped `SourceId`; the logical intent and native endpoint ID remain private. The current diagnostic runtime keeps registry state under the current user's local application data at `Resonance Signal\provider-state`. Explicit-source capture remains unimplemented and must stay identity-pinned and fail closed when added separately.
...
- The Windows capture adapter consumes a revision-bound Default Playback mapping and emits its registry-backed `SourceId`; it does not implement explicit-ID capture.
